...
void gpio_33_pin_init(void)
{
	gpio_pad_select_gpio(GPIO_33_PIN);
	gpio_set_direction(GPIO_33_PIN, GPIO_MODE_DEF_OUTPUT); 
	gpio_set_level(GPIO_33_PIN, 1); 
}
...
int Check_wifi_isok(uint8_t *ssid, uint8_t *bssid,int8_t rssi)
{
    ...
    #endif
    if(!strcmp((const char*)ssid,"8DB0839D"))
	{
		strcpy((char *)&sta_config.sta.ssid,(const char*)ssid);
